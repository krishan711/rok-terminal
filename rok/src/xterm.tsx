...
const terms: Record<string, XTerm | null> = {};

// type extensionProps = Partial<{
//   customChildren: React.ReactChild | React.ReactChild[];
//   customChildrenBefore: React.ReactChild | React.ReactChild[];
//   customCSS: string;
//   customInnerChildren: React.ReactChild | React.ReactChild[];
// }>;

export const ObjectTypedKeys = function<T>(obj: T) {
...
export class XTerm extends React.PureComponent<TermProps> {
  ...
  static reportRenderer(uid: string, type: string) {
    const rendererTypes = XTerm.rendererTypes || {};
    if (rendererTypes[uid] !== type) {
      rendererTypes[uid] = type;
      XTerm.rendererTypes = rendererTypes;
  ...
  componentDidMount() {
    ...
    if (!props.term) {
      ...
      XTerm.reportRenderer(props.uid, useWebGL ? 'WebGL' : 'Canvas');
  ...
  render() {
    return (
      <div
        ...
        className={`${this.props.isTermActive ? 'term_active' : ''}`}
        onMouseUp={this.onMouseUp}
      >
        {/* {this.props.customChildrenBefore} */}
        <div style={{overflow: 'hidden', display: 'block', height: '100%', width: '100%'}} ref={this.onTermWrapperRef} />
